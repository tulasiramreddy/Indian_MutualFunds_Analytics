"""SQLite database management for MF Analytics."""
...
def upsert_fund(conn, scheme_code: int, scheme_name: str,
    ...
    conn.execute("""
        ...
        ON CONFLICT(scheme_code) DO UPDATE SET
            ...
            scheme_category=COALESCE(excluded.scheme_category, scheme_category),
            scheme_type=COALESCE(excluded.scheme_type, scheme_type),
            fund_house=COALESCE(excluded.fund_house, fund_house),
            isin_growth=COALESCE(excluded.isin_growth, isin_growth),
            isin_div=COALESCE(excluded.isin_div, isin_div)
...
def bulk_insert_benchmark(conn, records: list[tuple]):
    ...
    """
    conn.executemany("""
        INSERT OR IGNORE INTO benchmark_data (index_name, date, close_price)
        VALUES (?, ?, ?)
    """, records)
...
def get_last_benchmark_date(conn, index_name: str) -> Optional[str]:
    ...
    row = conn.execute(
        "SELECT MAX(date) FROM benchmark_data WHERE index_name = ?",
        (index_name,)
    ).fetchone()
    return row[0] if row and row[0] else None


def get_fund_nav(scheme_code: int, db_path: str = DB_PATH) -> pd.DataFrame:
    """Get NAV history for a fund as a DataFrame."""
    with get_connection(db_path) as conn:
        df = pd.read_sql_query(
            "SELECT date, nav FROM nav_history WHERE scheme_code = ? ORDER BY date",
            conn, params=(scheme_code,), parse_dates=["date"]
        )
    return df


def get_benchmark_data(index_name: str, db_path: str = DB_PATH) -> pd.DataFrame:
    """Get benchmark price history as a DataFrame."""
    with get_connection(db_path) as conn:
        df = pd.read_sql_query(
            "SELECT date, close_price FROM benchmark_data WHERE index_name = ? ORDER BY date",
            conn, params=(index_name,), parse_dates=["date"]
        )
    return df


def get_all_funds(db_path: str = DB_PATH) -> pd.DataFrame:
    """Get all funds metadata."""
    with get_connection(db_path) as conn:
        df = pd.read_sql_query("SELECT * FROM funds ORDER BY scheme_name", conn)
    return df


def get_equity_funds(db_path: str = DB_PATH) -> pd.DataFrame:
...
def get_funds_with_nav(db_path: str = DB_PATH) -> pd.DataFrame:
...
def get_fund_houses(db_path: str = DB_PATH) -> list[str]:
...
def log_update_start(conn, update_type: str) -> int:
    ...
    cursor = conn.execute(
        "INSERT INTO update_log (update_type, started_at) VALUES (?, ?)",
...
def log_update_finish(conn, log_id: int, schemes_updated: int, status: str = "completed"):
    ...
    conn.execute(
        "UPDATE update_log SET finished_at=?, schemes_updated=?, status=? WHERE id=?",
